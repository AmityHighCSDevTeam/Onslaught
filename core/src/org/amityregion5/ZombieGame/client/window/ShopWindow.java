...
public class ShopWindow implements Screen {
	...
	public void drawMainRegion(float delta) {
		...
		for (int i = 0; i < cache.size(); i++) {
			...
			if (clickX > boxX && Gdx.graphics.getHeight() - clickY > boxY && clickX < boxX + purchaseableHeight
		...
		if (mouseOverIndex != -1) {
			...
			if (purchaseable.getCurrentLevel(player) > -1) {
				owned = new GlyphLayout(ZombieGame.instance.mainFont, "Owned", Color.BLUE, 300, Align.left, true);
	...
	public void drawInfoRegion(float delta) {
		...
		if (y < 100) {
			...
			shapeRender.setColor(0.7f, 0.7f, 0.7f, 1f);
			shapeRender.rect(screen.getWidth() - 120, getSecScrollBarPos(), 20, getSecScrollBarHeight());
