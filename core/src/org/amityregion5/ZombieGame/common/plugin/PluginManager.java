...
public class PluginManager {
	...
	public List<IPurchaseable> getPurchaseables() {
		return plugins.parallelStream().filter(PluginContainer::isActive).flatMap((p) -> p.getPurchaseables().stream())
				.collect(Collectors.toList());
	}

	public List<IWeapon> getActivatedWeapons() {
		return plugins.parallelStream().filter(PluginContainer::isActive).flatMap((p) -> p.getWeapons().stream())
				.collect(Collectors.toList());
	}
	/*
	 * public List<BuffApplicator> getBuffApplicators() {
	 * return
	 * plugins.parallelStream().filter(PluginContainer::isActive).flatMap((p)->p.getBuffApplicators().stream()).collect(
	 * Collectors.toList());
	 * }
	 */

	public List<Class<? extends IWeapon>> getActivatedWeaponClasses() {
		return plugins.parallelStream().filter(PluginContainer::isActive).flatMap((p) -> p.getWeaponClasses().stream())
				.collect(Collectors.toList());
	}
