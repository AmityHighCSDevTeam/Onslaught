...
public class TutorialGame extends Game {
	...
	public void tick(float deltaTime) {
		...
		if (tutorialPart == -1) {
			...
			String moveDirName = 
					ZombieGame.instance.settings.getInput("Move_Up").getName() +
			...
			getSingleplayerPlayer().setMoney(ZombieGame.instance.pluginManager.getActivatedWeapons().parallelStream().filter((w)->w.getName().equalsIgnoreCase("M9")).findFirst().get().getWeaponData(0).getPrice());
			...
			WeaponData m9Data = ((WeaponData)ZombieGame.instance.pluginManager.getActivatedWeapons().parallelStream().filter((w)->w.getName().equalsIgnoreCase("M9")).findFirst().get().getWeaponData(0));
			...
			addParticleToWorld(particle);
			tutorialPart = 4;
		} else if (tutorialPart == 4 && getSingleplayerPlayer().getWeapons().parallelStream().filter((w)->w.getWeapon().getName().equalsIgnoreCase("M9")).findFirst().get().getTotalAmmo() > 0) {
			removeParticle(particle);

			particle = new TextParticle(0, 1, this, "Press " + ZombieGame.instance.settings.getInput("Reload").getName() + " to reload.");
			addParticleToWorld(particle);
			tutorialPart = 5;
		} else if (tutorialPart == 5 && getSingleplayerPlayer().getWeapons().parallelStream().filter((w)->w.getWeapon().getName().equalsIgnoreCase("M9")).findFirst().get().getAmmo() > 0) {
			removeParticle(particle);
			...
			hostiles++;
			addEntityToWorld(model, getSingleplayerPlayer().getEntity().getBody().getWorldCenter().x + 5, getSingleplayerPlayer().getEntity().getBody().getWorldCenter().y);

			tutorialPart = 6;
		} else if (tutorialPart == 6 && hostiles == 0) {
			removeParticle(particle);

			particle = new TextParticle(0, 1, this, "Select the second hotbar slot by pressing " + ZombieGame.instance.settings.getInput("Hotbar_2").getName() + ".");
			addParticleToWorld(particle);
			tutorialPart = 7;
		} else if (tutorialPart == 7 && getSingleplayerPlayer().getCurrWeapIndex() == 1) {
			removeParticle(particle);

			particle = new TextParticle(0, 1, this, "Open your inventory by pressing " + ZombieGame.instance.settings.getInput("Inventory_Window").getName() + ".");
			addParticleToWorld(particle);
			tutorialPart = 8;
		} else if (tutorialPart == 8 && getSingleplayerPlayer().getScreen().getCurrentWindow() instanceof InventoryWindow) {
			removeParticle(particle);

			getSingleplayerPlayer().getWeapons().add(new WeaponStack(ZombieGame.instance.pluginManager.getActivatedWeapons().parallelStream().filter((w)->w.getName().equals("AK47")).findFirst().get()));
			...
			particle = new TextParticle(0, 1, this, "Press " + ZombieGame.instance.settings.getInput("Toggle_Flashlight").getName() + " to toggle your flashlight.");
			addParticleToWorld(particle);
			tutorialPart = 11;
			...
			particle = new TextParticle(0, 1, this, "Press " + ZombieGame.instance.settings.getInput("Close_Window").getName() + " to open the pause/quit window.");
			addParticleToWorld(particle);
			tutorialPart = 13;
