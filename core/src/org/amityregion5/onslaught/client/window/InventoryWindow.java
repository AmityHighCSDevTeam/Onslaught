...
public class InventoryWindow implements Screen {
	...
	private void drawMain(float delta) {
		...
		float y = (float) (screen.getHeight() - 101*Onslaught.getYScalar() + scrollPos);
		float w = screen.getWidth() - 221*Onslaught.getXScalar();
		...
		float trueWeaponBoxSize = (weaponBoxSize + weaponBoxBorder + weaponBoxBorder)*Onslaught.getXScalar();
		//Column numbers
		...
		//Clip the screen
		Rectangle clipBounds = new Rectangle(x, 100 * Onslaught.getYScalar(), w, screen.getHeight() - 201 * Onslaught.getYScalar());
		...
		for (int i = 0; i < player.getWeapons().size(); i++) {
			...
			shapeRender.rect(boxX, boxY, weaponBoxSize*Onslaught.getXScalar(), weaponBoxSize*Onslaught.getXScalar());
			...
			s.setBounds(boxX, boxY, weaponBoxSize*Onslaught.getXScalar(), weaponBoxSize*Onslaught.getXScalar());
			s.draw(batch);
			...
			if (mouseX > boxX && mouseY > boxY && mouseX < boxX + weaponBoxSize*Onslaught.getXScalar() && mouseY < boxY + weaponBoxSize*Onslaught.getXScalar()) {
				mouseOverIndex = i;
			}
		}
		//Unclip the screen
		...
		if (mouseOverIndex != -1) {
			...
			//Calculate name
			GlyphLayout nameGlyph = new GlyphLayout(Onslaught.instance.mainFont, weapon.getWeapon().getName(), 0, weapon.getWeapon().getName().length(),
					Color.BLACK, 300*Onslaught.getXScalar(), Align.left, false, "...");

			boxWidth = Math.max(boxWidth, nameGlyph.width + 8);
			boxHeight += nameGlyph.height + 8*Onslaught.getYScalar();

			//Calculate description
			GlyphLayout descGlyph = new GlyphLayout(Onslaught.instance.mainFont, weapon.getWeapon().getDescription(), Color.BLACK, 300*Onslaught.getXScalar(), Align.left, true);

			boxWidth = Math.max(boxWidth, descGlyph.width + 8);
			boxHeight += descGlyph.height + 4*Onslaught.getYScalar();
			...
			//Name
			Onslaught.instance.mainFont.draw(batch, nameGlyph, mouseX + 4, textDrawing);
			textDrawing -= nameGlyph.height + 4;

			textDrawing -= 10;

			//Description
			Onslaught.instance.mainFont.draw(batch, descGlyph, mouseX + 4, textDrawing);
	...
	private double getMaxScrollAmount() {
		return (weaponBoxSize / ((int) ((screen.getWidth() - 221*Onslaught.getXScalar()) / (weaponBoxSize + weaponBoxBorder + weaponBoxBorder))) + 2)
